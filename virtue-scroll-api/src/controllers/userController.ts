...
export class UserController {
	constructor(private userService: UserService, private corsHeaders: HeadersInit) {}
	...
	private validateUserInput(user: Partial<User>): boolean {
		return Boolean(user.username?.trim() && user.display_name?.trim() && user.email?.trim() && user.id);
	}
